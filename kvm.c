...
static int kvm__check_extensions(struct kvm *self)
{
    ...
    for (i = 0; i < ARRAY_SIZE(kvm_req_ext); i++) {
        if (!kvm__supports_extension(self, kvm_req_ext[i].code)) {
            error("Unsuppored KVM extension detected: %s", kvm_req_ext[i].name);
            return (int)-i;
        }
    }
...
struct kvm *kvm__init(const char *kvm_dev, unsigned long ram_size)
{
    ...
    if (self->sys_fd < 0) {
        if (errno == ENOENT)
            die("'%s' not found. Please make sure your have CONFIG_KVM enabled \
            and that the KVM modules are loaded.", kvm_dev);
...
#define TIMER_INTERVAL_NS 1000000 /* 1msec */
