...
struct virt_queue {
    ...
    uint32_t pfn;
    /*
     * The last_avail_ndx field is an index to ->ring of struct vring_avail.
     * It's where we assume the next request index is at.
     */
    uint16_t last_avail_ndx;
};
...
static bool blk_virtio_out(struct kvm *self, uint16_t port, void *data, int size, uint32_t count)
{
    ...
    case VIRTIO_PCI_QUEUE_NOTIFY: {
        ...
        if (queue->vring.avail->idx == queue->last_avail_ndx) {
            warning("vring is empty");
            break;
        }

        desc_ndx = queue->vring.avail->ring[queue->last_avail_ndx++ % queue->vring.num];
